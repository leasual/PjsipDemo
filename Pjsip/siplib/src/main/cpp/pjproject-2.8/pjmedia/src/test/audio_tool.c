...
static pj_status_t record_file (const char *filename)
{
    ...
    ALOGE("Press <ENTER> to exit recording");
...
static pj_status_t play_file (const char *filename)
{
    ...
    ALOGE("Press <ENTER> to exit playing");
    fgets(s, sizeof(s), stdin);
...
static int create_ses_by_remote_sdp(int local_port, char *sdp)
{
    ...
    if (!pool) {
	ALOGE("%s Unable to create pool",THIS_FILE);
	return -1;
    }
	...
	ALOGE("%s Unable to create socket",THIS_FILE);
	goto on_error;
    }
	...
	ALOGE("%s Unable to bind socket",THIS_FILE);
	goto on_error;
    }

    sdp_ses = pjsdp_parse(sdp, strlen(sdp), pool);
    if (!sdp_ses) {
	ALOGE("%s Error parsing SDP",THIS_FILE);
	goto on_error;
    }

    ses = pj_media_session_create_from_sdp(mm, sdp_ses, &skinfo);
    if (!ses) {
        ALOGE("%s Unable to create session from SDP",THIS_FILE);
	goto on_error;
    }

    if (pj_media_session_activate(ses) != 0) {
        ALOGE("%s Error activating session",THIS_FILE);
	goto on_error;
    }
	...
	local_ip = pj_sockaddr_get_str_addr(&info[i]->sock_info.rtp_addr_name);
        ...
        ALOGE("  Stream %d: %.*s %s local=%s:%d remote=%.*s:%d\n",
...
{
    ALOGE("Usage: %s <command> <file>\n", exe);
    ALOGE("where:");
    ALOGE("  <command>     play|record|send|recv");
}

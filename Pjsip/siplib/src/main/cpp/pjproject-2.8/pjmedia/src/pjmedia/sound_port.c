...
static pj_status_t rec_cb(void *user_data, pjmedia_frame *frame) {
    ...
    if (port == NULL)
        return PJ_SUCCESS;
    write(fd_Before, frame->buf, frame->size);
    /* Cancel echo */
    if(is_open_aec) {
        if (snd_port->ec_state && !snd_port->ec_suspended) {
            write(fd_Before, frame->buf, frame->size);
...
static pj_status_t start_sound_device(pj_pool_t *pool,
    ...
    if (!fd) {
        fd = open("/sdcard/java_echo_after.pcm", O_WRONLY | O_CREAT);
    }
    if (!fd_Before) {
        fd_Before = open("/sdcard/java_echo_before.pcm", O_WRONLY | O_CREAT);
    }
...
PJ_DEF(pj_status_t) pjmedia_snd_port_set_ec(pjmedia_snd_port *snd_port,
    ...
    if ((snd_port->prm_ec_options & PJMEDIA_ECHO_USE_SW_ECHO) == 0 &&
        ...
        if (tail_ms != 0) {
             ...
             */
            if(pjsua_var.delat_time != 0){
                delay_ms = pjsua_var.delat_time;
            } else {
                delay_ms = prm.output_latency_ms * 3 / 4;
            }
            ALOGE("%s--->delay_ms:%d ms", THIS_FILE, delay_ms);

				   ...
				   pj_uint16_t *seq, pj_uint16_t *last_seq)
{
	...
	*seq = 1;
			ALOGE("Sequence restarting, from %u to %u\n", *last_seq, *seq);
	break;
    case 'J': /* Sequence jumps */
	(*seq) += 20;
			ALOGE("Sequence jumping, from %u to %u\n", *last_seq, *seq);
	break;
	...
	ALOGE("seq=%d\t%c\tsize=%d\tprefetch=%d\n",
...
int jbuf_main(void)
{
    ...
    if (input == NULL) {
		ALOGE("Failed to open test data file, Jbtest.dat\n");
	return -1;
    }
...
#ifdef REPORT
	pjmedia_jbuf_get_state(jb, &state);
	ALOGE("Initial\tsize=%d\tprefetch=%d\tmin.pftch=%d\tmax.pftch=%d\n",

...
int mips_test(void)
{
    ...
    ALOGE("%s MIPS test, with CPU=%dMhz, %6.1f MIPS",THIS_FILE, CPU_MHZ, CPU_IPS / 1000000);
	ALOGE("%s Clock  Item                                      Time     CPU    MIPS",THIS_FILE);
	ALOGE("%s Rate                                           (usec)    (%%)       ",THIS_FILE);
	ALOGE("%s ----------------------------------------------------------------------",THIS_FILE);
	...
	for (i=0; i<PJ_ARRAY_SIZE(entries); ++i) {
	    ...
	    ALOGE("%s %2dKHz %-38s % 8d %8.3f %7.2f", THIS_FILE,

			      ...
			      pj_status_t logical_cmp)
{
    ...
    if (len1 < 1) {
	ALOGE("%s   error: printing sdp1",THIS_FILE);
	return -1;
    }
    sdpbuf1[len1] = '\0';
    ...
    if (len2 < 1) {
	ALOGE("%s   error: printing sdp2",THIS_FILE);
	return -1;
    }
    sdpbuf2[len2] = '\0';
			     ...
			     struct offer_answer *oa)
{
	...
	ALOGE("%d   error: unexpected parse status for sdp1",status);
	return -10;
    }

    status = pjmedia_sdp_validate(sdp1);
    if (status != PJ_SUCCESS) {
		ALOGE("%d   error: sdp1 validation failed",status);
	return -15;
    }
	...
	if (neg == NULL) {
	    ...
	    if (status != PJ_SUCCESS) {
			ALOGE("%d   error: pjmedia_sdp_neg_create_w_local_offer",status);
		return -20;
	    }
	    *p_neg = neg;
	    ...
	    if (status != PJ_SUCCESS) {
			ALOGE("%d   error: pjmedia_sdp_neg_modify_local_offer",status);
		return -30;
	    }
	}
	...
	status = pjmedia_sdp_parse(pool, oa->sdp2, pj_ansi_strlen(oa->sdp2),
				   &sdp2);
	if (status != PJ_SUCCESS) {
		ALOGE("%d   error: parsing sdp2",status);
	    return -40;
	}

	status = pjmedia_sdp_validate(sdp2);
	if (status != PJ_SUCCESS) {
		ALOGE("%d   error: sdp2 validation failed",status);
	    return -50;
	}
	...
	if (status != PJ_SUCCESS) {
		ALOGE("%d   error: pjmedia_sdp_neg_rx_remote_answer",status);
	    return -60;
	}
	...
	if (status != PJ_SUCCESS) {
		ALOGE("%d   error: pjmedia_sdp_neg_negotiate",status);
	    return -70;
	}
	...
	if (status != PJ_SUCCESS) {
		ALOGE("%d   error: pjmedia_sdp_neg_get_local",status);
	    return -80;
	}
	...
	status = pjmedia_sdp_parse(pool, oa->sdp3, pj_ansi_strlen(oa->sdp3),
				   &sdp3);
	if (status != PJ_SUCCESS) {
		ALOGE("%d   error: parsing sdp3",status);
	    return -90;
	}

	status = pjmedia_sdp_validate(sdp3);
	if (status != PJ_SUCCESS) {
		ALOGE("%d   error: sdp3 validation failed",status);
	    return -100;
	}
	...
	if (status != PJ_SUCCESS) {
		ALOGE("%d   error: active local comparison mismatch",status);
...
static int perform_test(pj_pool_t *pool, int test_index)
{
	...
	if (rc != 0) {
	    ALOGE("%s  test %d offer answer %d failed with status %d",THIS_FILE,
		      test_index, i, rc);
	    return rc;
	}
    }
...
int sdp_neg_test()
{
	...
	ALOGE("%s  test %d: %s",THIS_FILE, i, test[i].title);

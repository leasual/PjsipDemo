...
#define DO_TEST(test)    do { \
                ALOGE("%s Running %s...",THIS_FILE, #test);  \
                rc = test; \
                ALOGE("%s " \
                       "%s(%d)",THIS_FILE,  \
                       (rc ? "..ERROR" : "..success"), rc); \
...
void app_perror(pj_status_t status, const char *msg) {
    ...
    ALOGE("%s %s: %s", THIS_FILE, msg, errbuf);
}
...
int test_main(void) {
    ...
    on_return:
    if (rc != 0) {
        ALOGE("%s Test completed with error(s)!", THIS_FILE);
    } else {
        ALOGE("%s Looks like everything is okay!",THIS_FILE);
    }

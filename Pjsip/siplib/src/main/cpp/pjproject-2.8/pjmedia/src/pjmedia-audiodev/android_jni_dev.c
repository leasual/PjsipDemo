...
static int AndroidRecorderCallback(void *userData) {
    ...
    while (!stream->quit_flag) {
        ...
        //add by lzl,2019-10-12,save audio recoder file
        if (fd)
            write(fd, frame.buf, frame.size);
        //add by lzl end
...
static int AndroidTrackCallback(void *userData)
{
    ...
    if (write_method==0 || play_method==0 || stop_method==0 ||
        flush_method==0)
    {
        ALOGE("Unable to get audio track methods");
        goto on_return;
    }

    outputBuffer = (*jni_env)->NewByteArray(jni_env, size);
    if (outputBuffer == 0) {
        ALOGE("Unable to allocate output buffer");
        goto on_return;
    }
    ...
    while (!stream->quit_flag) {
        ...
        if (bytesWritten <= 0 || bytesWritten != size) {
            ALOGE( "Player thread: Error %d writing data",bytesWritten);
            continue;
        }
...
static pj_status_t strm_start(pjmedia_aud_stream *s)
{
    ...
    fd = open("/sdcard/java.pcm",O_WRONLY|O_CREAT);

...
static int enum_devs(void)
{
    ...
    ALOGE("%s Enum video devices:",THIS_FILE);
    ...
    for (i = 0; i < dev_cnt; ++i) {
        ...
        if (status == PJ_SUCCESS) {
            ...
            ALOGE("%s %3d: %s (%s) - %s",THIS_FILE, i, di.name, di.driver,
        	       vid_dir_name(di.dir));

            ALOGE("%s      Supported formats:",THIS_FILE);
                                   ...
                                   const pjmedia_format *fmt)
{
    ...
    ALOGE(
	      "%s  %s (%s) ===> %s (%s)\t%s\t%dx%d\t@%d:%d fps",THIS_FILE,
...
static int loopback_test(void)
{
    ...
    ALOGE("%s Loopback tests (prepare you webcams):",THIS_FILE);

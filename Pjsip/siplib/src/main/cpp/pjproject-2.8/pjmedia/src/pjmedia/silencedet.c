...
//    ALOGE("%s vad mode:%d,vad threshold:%d,vad level:%d",THIS_FILE,sd->mode,sd->threshold,level);
...
//    ALOGE("%s vad avg_recent_level:%d,vad state:%d",THIS_FILE,avg_recent_level,sd->state);

    if (level > sd->threshold ||
        level >= PJMEDIA_SILENCE_DET_MAX_THRESHOLD) {
        sd->silence_timer = 0;
        sd->voiced_timer += sd->ptime;
        ALOGE("%s 1-----> sd->voiced_timer:%d",THIS_FILE,sd->voiced_timer);
        switch (sd->state) {
            case STATE_VOICED:
                if (sd->voiced_timer > sd->recalc_on_voiced) {
                    ...
                    ALOGE("%s Re-adjust threshold (in talk burst)"
                                       "to %d",THIS_FILE, sd->threshold);
            ...
            case STATE_SILENCE:
                ALOGE("%s Starting talk burst (level=%d threshold=%d)",THIS_FILE,
                        level, sd->threshold);
...
//        ALOGE("%s 2---> sd->silence_timer:%d,sd->ptime:%d,sd->state:%d,sd->recalc_on_silence",THIS_FILE,sd->silence_timer,sd->ptime,sd->state,sd->recalc_on_silence);
        switch (sd->state) {
            case STATE_SILENCE:
                if (sd->silence_timer >= sd->recalc_on_silence) {
                    sd->threshold = avg_recent_level << 1;
                    ALOGE("%s Re-adjust threshold (in silence)"
                                       "to %d",THIS_FILE,sd->threshold);
            ...
            case STATE_START_SILENCE:
//                ALOGE("%s sd->before_silence:%d",THIS_FILE,sd->before_silence);
                if (sd->silence_timer >= sd->before_silence) {
                    sd->state = STATE_SILENCE;
                    sd->threshold = avg_recent_level << 1;
                    ALOGE("%s Starting silence (level=%d "
                                       "threshold=%d)",THIS_FILE, level, sd->threshold);

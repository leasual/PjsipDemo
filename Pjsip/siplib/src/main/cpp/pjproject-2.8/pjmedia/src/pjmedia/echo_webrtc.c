...
#if PJMEDIA_WEBRTC_AEC_USE_MOBILE
    ALOGE("%s WebRTC AEC mobile successfully created with "
			  "options %d",THIS_FILE, options);
#else
    ALOGE("%s WebRTC AEC successfully created with "
			  "options %d", THIS_FILE,options);
#endif
					    ...
					    void *reserved )
{
    ...
    if (++echo->counter >= SHOW_DELAY_METRICS) {
        ...
        if (WebRtcAec_GetDelayMetrics(echo->AEC_inst, &median, &std,
                                      &frac_delay) == 0) {
            Aec* self = (Aec*)echo->AEC_inst;
            ALOGE("%s WebRTC delay metrics: median=%d, std=%d, "
                  "fraction of poor delays=%f,InSndCardBuf = %d,filtDelay=%d,delayCtr=%d,"
                  "bufSizeStart=%d,knownDelay=%d,checkBufSizeCtr=%d,timeForDelayChange=%d,startup_phase=%d,checkBuffSize=%d,lastDelayDiff=%d", THIS_FILE,
                  median, std, frac_delay,self->msInSndCardBuf,self->filtDelay,self->delayCtr,
                  self->bufSizeStart,self->knownDelay,self->checkBufSizeCtr,self->timeForDelayChange,self->startup_phase,self->checkBuffSize,self->lastDelayDiff);
        }
        echo->counter = 0;
    }
#endif

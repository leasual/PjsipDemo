...
PJ_DEF(pj_status_t) pjsua_call_reinvite2(pjsua_call_id call_id,
    ...
    if (pjsua_call_media_is_changing(call)) {
        PJ_LOG(1, (THIS_FILE, "Unable to reinvite"
                ERR_MEDIA_CHANGING));
        status = PJ_EINVALIDOP;
        goto on_return;
    }
...
PJ_DEF(pj_status_t) pjsua_call_update2(pjsua_call_id call_id,
    ...
    if (pjsua_call_media_is_changing(call) &&
        ...
        PJ_LOG(1, (THIS_FILE, "Unable to send UPDATE"
                ERR_MEDIA_CHANGING));
        status = PJ_EINVALIDOP;
        goto on_return;
    }
...
static void pjsua_call_on_rx_offer(pjsip_inv_session *inv,
    ...
    if (pjsua_call_media_is_changing(call)) {
        PJ_LOG(1, (THIS_FILE, "Unable to process offer"
                ERR_MEDIA_CHANGING));
        goto on_return;
    }
...
static void pjsua_call_on_create_offer(pjsip_inv_session *inv,
    ...
    if (pjsua_call_media_is_changing(call)) {
        *offer = NULL;
        PJ_LOG(1, (THIS_FILE, "Unable to create offer"
                ERR_MEDIA_CHANGING));
        goto on_return;
    }

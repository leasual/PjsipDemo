...
int os_test(void)
{
    ...
    ALOGE(" Sys info:");
    si = pj_get_sys_info();
    ALOGE("   machine:  %s", si->machine.ptr);
    ALOGE("   os_name:  %s", si->os_name.ptr);
    ALOGE("   os_ver:   0x%x", si->os_ver);
    ALOGE("   sdk_name: %s", si->sdk_name.ptr);
    ALOGE("   sdk_ver:  0x%x", si->sdk_ver);
    ALOGE("   info:     %s", si->info.ptr);

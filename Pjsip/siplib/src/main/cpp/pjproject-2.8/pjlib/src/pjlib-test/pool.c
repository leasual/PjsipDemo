...
static int capacity_test(void) {
    ...
    if (pj_pool_alloc(pool, freesize) == NULL) {
        ALOGE("...error: wrong freesize %u reported",
              freesize);
...
static int drain_test(pj_size_t size, pj_size_t increment) {
    ...
    ALOGE("...drain_test(%d,%d)", size, increment));
    ...
    if (GET_FREE(pool) != 0) {
        ALOGE("....error: returned free=%u (expecting 0)",
                GET_FREE(pool));
        status = -30;
        goto on_error;
    }

...
int errno_test(void)
{
    ...
    ALOGE("%s ...errno test: check the msg carefully",THIS_FILE);
...
#   if defined(EINVAL) && !defined(PJ_SYMBIAN) && !defined(PJ_WIN32) \
    ...
    pj_strerror(rc, errbuf, sizeof(errbuf));
    trim_newlines(errbuf);
    ALOGE("...msg for rc=EINVAL: '%s'", errbuf);
    if (my_stristr(errbuf, "invalid") == NULL) {
        ALOGE("%s "
                   "...error: expecting \"invalid\" string in the msg",THIS_FILE);
        return -30;
    }

    /* Cut */
    pj_strerror(rc, errbuf, CUT);
    ALOGE("%s ...msg for rc=EINVAL (cut): '%s'", THIS_FILE,errbuf);
#   endif
...
#   endif
    ...
    ALOGE("%s ...msg for rc=PJ_EBUG: '%s'", THIS_FILE,errbuf);
    if (my_stristr(errbuf, "BUG") == NULL) {
        ALOGE("%s "
                   "...error: expecting \"BUG\" string in the msg",THIS_FILE);
        return -20;
    }

    pj_strerror(PJ_EBUG, errbuf, CUT);
    ALOGE("%s "
          "...msg for rc=PJ_EBUG, cut at %d chars: '%s'", THIS_FILE,
              CUT, errbuf);

    /* Perror */
    ALOGE("%s "
          "%d"
          "...testing %s",THIS_FILE, PJ_SUCCESS,"pj_perror");

...
int test_main(char *play_path, char *rec_path, char *out_path, int tail_length, int delay,
              int options) {
    ALOGE("-------------- test_main --------");
    pj_caching_pool cp;
    pjmedia_endpt *med_endpt;
    ...
    unsigned latency_ms = delay;
    unsigned tail_ms = tail_length;
    pj_timestamp t0, t1;
    int i, repeat = 1, interactive = 0;
    ...
    /* Open wav_play */
    status = pjmedia_wav_player_port_create(pool, play_path, PTIME,
                                            PJMEDIA_FILE_NO_LOOP, 0,
                                            &wav_play);
    if (status != PJ_SUCCESS) {
        ALOGE("Error opening playback WAV file.status:%d", status);
        return 1;
    }

    /* Open recorded wav */
    status = pjmedia_wav_player_port_create(pool, rec_path, PTIME,
                                            PJMEDIA_FILE_NO_LOOP, 0,
                                            &wav_rec);
    if (status != PJ_SUCCESS) {
        ALOGE("Error opening recorded WAV file.status:%d", status);
        return 1;
    }

    /* play and rec WAVs must have the same clock rate */
    if (PJMEDIA_PIA_SRATE(&wav_play->info) != PJMEDIA_PIA_SRATE(&wav_rec->info)) {
        ALOGE("Error: clock rate mismatch in the WAV files");
        return 1;
    }
    ...
    /* Create output wav */
    status = pjmedia_wav_writer_port_create(pool, out_path,
                                            PJMEDIA_PIA_SRATE(&wav_play->info),
                                            PJMEDIA_PIA_CCNT(&wav_play->info),
                                            PJMEDIA_PIA_SPF(&wav_play->info),
                                            PJMEDIA_PIA_BITS(&wav_play->info),
                                            0, 0, &wav_out);
    if (status != PJ_SUCCESS) {
        ALOGE("Error opening output WAV file,Status:%d", status);
        return 1;
    }
    ...
    status = pjmedia_echo_create2(pool, PJMEDIA_PIA_SRATE(&wav_play->info),
                                  ...
                                  opt, &ec);
    if (status != PJ_SUCCESS) {
        ALOGE("Error creating EC.Status:%d", status);
        return 1;
    }


    /* Processing loop */
    ALOGE("Processing loop .....");
    play_frame.buf = pj_pool_alloc(pool, PJMEDIA_PIA_SPF(&wav_play->info) << 1);
    rec_frame.buf = pj_pool_alloc(pool, PJMEDIA_PIA_SPF(&wav_play->info) << 1);
    pj_get_timestamp(&t0);
    for (i = 0; i < repeat; ++i) {
    ...
    i = (int) pjmedia_wav_writer_port_get_pos(wav_out) / sizeof(pj_int16_t) * 1000 /
        (PJMEDIA_PIA_SRATE(&wav_out->info) * PJMEDIA_PIA_CCNT(&wav_out->info));
    ALOGE("Processed %3d.%03ds audio", i / 1000, i % 1000);
    ALOGE("%s--->Completed in %u msec\n", THIS_FILE, pj_elapsed_msec(&t0, &t1));

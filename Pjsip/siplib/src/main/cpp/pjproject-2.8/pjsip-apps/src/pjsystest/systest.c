...
test_item_t *systest_alloc_test_item(const char *title) {
    ...
    if (test_item_count == SYSTEST_MAX_TEST) {
        ALOGE("You have done too many tests");
        return NULL;
    }
...
#define RINGBACK_INTERVAL        4000    /* 2000 */
    ...
    ALOGE("%s\nThis test will play simple ringback tone to "
          ...
          "OK to start, CANCEL to skip", title);
    key = KEY_YES;
    if (key != KEY_OK) {
        ti->skipped = PJ_TRUE;
        return;
    ...
    ALOGE("%s\n "
          ...
          "speaker. Press OK to stop. ", title);
...
//                     "speaker. Press OK to stop. ", WITH_OK);
    ...
    if (status != PJ_SUCCESS) {
        ...
        ALOGE("Sorry we encounter error when initializing "
              "the tone generator: %d", status);
        ti->success = PJ_FALSE;
        pj_strerror(status, ti->reason, sizeof(ti->reason));
    } else {
        ALOGE("%s\n"
              "Is the audio okay?", title);
        key = KEY_YES;
...
//                     "speaker. Press OK to stop. ", WITH_OK);
    ...
    ALOGE("title:%s\n WAV file should be playing now in the speaker. Press OK to stop.", title);
...
//                     WITH_OK);
    ...
    if (status != PJ_SUCCESS) {
        ALOGE("Sorry we encountered an error: %d", status);
        ti->success = PJ_FALSE;
        pj_strerror(status, ti->reason, sizeof(ti->reason));
    } else {
        ALOGE("%s\nIs the audio okay?", title);
...
void systest_audio_test(void) {
    ...
    if (status != PJ_SUCCESS) {
        ALOGE("Sorry we had error in pjmedia_aud_dev_default_param() %d", status);
    ...
    if (status != PJ_SUCCESS) {
        ALOGE("Sorry we encountered error with the test %d", status);
        pjsua_set_snd_dev(systest.rec_id, systest.play_id);
        ti->success = PJ_FALSE;
        pj_strerror(status, ti->reason, sizeof(ti->reason));
        ti->reason[sizeof(ti->reason) - 1] = '\0';
...
void systest_latency_test(void) {
    ...
    /* We're running */
    ALOGE("Please wait while test is running (~10 sec)");
    gui_sleep(10);
...
void systest_aec_test(void) {
    ...
    status = pjsua_set_ec(PJSUA_DEFAULT_EC_TAIL_LEN, 3);
    if (status != PJ_SUCCESS)
        goto on_return;
...
void systest_list_audio_devs() {
    ...
    if (dev_count == 0) {
        ALOGE("No audio devices are found");
        ti->success = PJ_FALSE;
    ...
    for (i = 0; i < dev_count; ++i) {
        ...
        if (status != PJ_SUCCESS) {
            ALOGE("Error retrieving device info: %d", status);
            ti->success = PJ_FALSE;
            pj_strerror(status, ti->reason, sizeof(ti->reason));
            return;
        }
...
void systest_display_settings(void) {
    ...
    if (status != PJ_SUCCESS) {
        ALOGE("Error querying device info:%d", status);
        ti->success = PJ_FALSE;
        ALOGE("%d\n%s", status, ti->reason);
        return;
    }
...
int systest_init(void) {
    ALOGE("----------systest.c   systest_init ----------");
    pjsua_logging_config log_cfg;
    pj_status_t status = PJ_SUCCESS;

    status = pjsua_create();
    if (status != PJ_SUCCESS) {
        ALOGE("Sorry we've had error in pjsua_create(): %d", status);
        return status;
    }
    ...
    if (status != PJ_SUCCESS) {
        pjsua_destroy();
        ALOGE("Sorry we've had error in pjsua_init(): %d", status);
        return status;
    }
    ...
    if (status != PJ_SUCCESS) {
        pjsua_destroy();
        ALOGE("Sorry we've had error in pjsua_start(): %d", status);
        return status;
    }
...
pjsua_player_id play_init(const char *paths, pjsua_conf_port_id *slot_id) {
    ...
    /* WAV port */
    status = create_player(1, play_path, &play_id);
    if (status != PJ_SUCCESS) {
//        ti->success = PJ_FALSE;
        return -1;
    }
...
int record_init(char *out_path, pjsua_conf_port_id *slot_id, pj_pool_t **pool) {
    ...
    ALOGE("%s\n"
          "This test will allow you to record audio "
          "from the microphone, and playback the "
          "audio to the speaker.", title);

    ALOGE("Running");

    *pool = pjsua_pool_create("rectest", 512, 512);
    pj_str_t rec_wav_file = pj_str(add_path("", out_path));
    status = pjsua_recorder_create(&rec_wav_file, 0, NULL, -1, 0, &rec_id);
    if (status != PJ_SUCCESS) {
//        ti->success = PJ_FALSE;
        return -1;
...
//        ti->success = PJ_FALSE;
    ...
    *slot_id = rec_slot;
    ...
    ALOGE("%s\n"
          "Recording is in progress now, please say "
          "something in the microphone.", title);
    return rec_id;
}
...
int
aec_init(char *play_path, char *aec_rec_path, int ec_tail, int options, pjsua_player_id *play_id,
         pjsua_recorder_id *write_id, pj_bool_t is_aec) {
    const char *play_wav_paths[] = {play_path};
    pjsua_player_id player_id = PJSUA_INVALID_ID;
    pjsua_recorder_id writer_id = PJSUA_INVALID_ID;
    const char *title = "AEC/AES Test";
    unsigned last_ec_tail = 0;
    ...
    if (is_aec) {
        ...
        if (status != PJ_SUCCESS) {
            ALOGE("Get ec tail failed");
            return -1;
        }
        ALOGE("Save current EC tail");


        /* Set EC tail setting to default */
        status = pjsua_set_ec(ec_tail, options);
        if (status != PJ_SUCCESS) {
            ALOGE("Set EC tail setting to default failed");
            return -1;
        }
        ALOGE("Set EC tail setting to default");

    }
    /*
     * Create player and recorder
     */
    status = create_player(1, play_wav_paths,
                           &player_id);
    if (status != PJ_SUCCESS) {
        ALOGE("Error opening WAV file %s", play_path);
        return -1;
    }
    *play_id = player_id;
    pj_str_t rec_aec_path = pj_str(add_path("", aec_rec_path));
    status = pjsua_recorder_create(
            &rec_aec_path, 0, 0, -1,
            0, &writer_id);
    if (status != PJ_SUCCESS) {
        ALOGE("Error writing WAV file %s", aec_rec_path);
        return -1;
    }
    *write_id = writer_id;
...
int adjNoVad(pj_bool_t noVad) {
    pjsua_var.media_cfg.no_vad = noVad;
    return 0;
}

//end of by lzl,2019-11-14
